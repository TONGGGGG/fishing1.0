...
<%--<c:if test="${not empty user }">--%>
        ...
        <div class="footer">
            <div class="footer_signup_link">
                <a class="signup_link" href="/ff/user/showlogin">Log in</a>
...
<script type="text/javascript">
        ...
        $('#signup_forms_submit').click(function(){
            if (flag) {
                if ($("#signup_email").val() == "" || $("#signup_password").val() == "" || $("#signup_confirm_password").val() == "" || $("#signup_username").val() == "") {
                    alert('email password and username mustn\'t be empty!');
                    ...
                    if ($("#signup_password").val() != $("#signup_confirm_password").val()) {
                        ...
                        var url = 'emailCheck?email=' + $("#signup_email").val() + '&uname=' + $("#signup_username").val() + '&passwd=' + $('#signup_password').val();
                        $.getJSON(url, function (data) {
                            if (data['state'] == 1) {
                                $("#signup_account").empty();
                                var s = '<div class="form_username"><input type="text" name="checkCode" placeholder="User" id="signup_username"></div>';
                                $("#signup_account").append(s);
                                $("#signup_forms_submit").addClass("regist");
                                flag=false;
                            } else {
                                alert("邮箱已被注册！");
                            }
    ...
    $(function(){
        $("#signup_forms_submit").click(function(){
            if (!flag){
                var url='regist?checkCode='+$('#signup_username').val();
                $.getJSON(url , function(data) {
                    if(data['state']=1){
                        alert("注册成功！");
                        $(location).attr('href', '${pageContext.request.contextPath}/user/showlogin');
                    }else {
                        alert("注册失败！");
                    }
                });
